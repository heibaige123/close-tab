...
export function ClosedTabItem({ tab, onDelete }: { tab: ClosedTab; onDelete?: () => void }) {
  return (
    <li className="min-w-0">
      <div className="group flex items-start gap-2.5 hover:bg-white/80 px-2 py-1.5 rounded-lg min-w-0 transition">
        <a
          href={tab.url}
          target="_blank"
        ...
        >
          {tab.title && (
            <>
              <span
                className="font-medium text-slate-900 group-hover:text-blue-700 whitespace-nowrap transition shrink"
                title={tab.title}
              >
                {tab.title}
              </span>
          ...
          <span className="text-slate-500 group-hover:text-blue-700 truncate transition" title={tab.url}>
            {tab.url}
          </span>
        </a>
        {onDelete && (
